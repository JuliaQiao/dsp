...
To rephrase, we're trying to find the probability that Elvis is an identical twin, given that he is a male twin. Let's assume that being an identical twin and gender are two independent variables--this is anecdotal knowledge.

To get that, we need to know the probability that he is a male twin and he is identical, and the probability that he is a male twin.

First, I calculated the probability of him being a male twin and identical:

1/300 * 1/2

Then, I calculated the porbability of fraternal twins who are both boys:

1/125* 1/2 * 1/2 

Adding those up gives us the probability of Elvis being a male twin. 

We divide the first calculation by the second:

(1/330*1/2) / ((1/330*1/2)+(1/125*1/2*1/2 ))

After doing the calculations, I get 5/11
...
Frequentist statistics conduct many trials or take samples from populations in order to come up with a probability of the observed instances. Frequentist probabilities focuses on events that are long term, repeated, and do not use probability to predict values that may be unknown or one time events.  They do not predict one time events or events that have not happened (ie. any event they cannot observe the result of). Bayesian statistics, on the other hand, is all about using probability to predict one time events and events that have not occured-- using new evidence to continuously update the hypothesis. In my interpretation, most of the probability that we see in data science is Bayesian--trying to predict probabilties of future events with what we have, while acknowledging we may not have all the information/know all the factors but do our best to continuously update our knowledge. Whereas frequentist probabiility would not apply to an instance where we try to use what we know to predict a future event because frequentists do not rely on prior probability. 
