...
Frequentist statistics focuses on events that are long term, repeated, and do not use probability to predict values that may be unknown or one time events. Frequentist statistics conduct many trials or take samples from populations in order to come up with a probability of the observed instances. They do not predict one time events or events that have not happened (ie. any event they cannot observe the result of). Bayesian statistics, on the other hand, is all about using probability to predict one time events and events that have not occured-- using new evidence to continuously update the hypothesis. In my interpretation, most of the probability that we see in data science is Bayesian--trying to predict probabilties of future events with what we have, while acknowledging we may not have all the information/know all the factors but do our best to continuously update our knowledge. Whereas frequentist probabiility would not apply to an instance where we try to use what we know to predict a future event because they do not rely on prior probability. 
